...
public class MongoBuildDAO implements BuildDAO {
	...
	private MapReduceOutput mapReduce() {
		...
		try {
			map = IOUtils.toString(classLoader
					.getResourceAsStream("js/MapReduce/mapTest.js"));
			reduce = IOUtils.toString(classLoader
					.getResourceAsStream("js/MapReduce/reduceTest.js"));
		} catch (IOException e) {
			e.printStackTrace();
		}
